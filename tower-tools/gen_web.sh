...
if [ ! -d "$app_home_dir/$1" ]; then
	mvn -B archetype:generate -DarchetypeCatalog=internal -DgroupId=com.$company.service.$projectid -DartifactId=$projectid
	cd $projectid
	rm -rf src
...
##config
mvn -B com.tower.soafw:tower-config-maven-plugin:2.0.0-SNAPSHOT:config -DartifactId=$projectid -DdestDir=../../projects -Dmodel=AllIn -DgenModule=web -DmoduleSuffix=$3 -Dcompany=$company -Dscop=web -X