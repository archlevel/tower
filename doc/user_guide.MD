...
### 前置条件
+ 安装好JDK
    + 设置好JAVA_HOME，并添加到path环境变量上
+ 安装好maven
    + 把maven的bin目录加到path环境变量上
...
### 启动mysql实例
+ docker run --name mysoa -p 3206:3306 -e MYSQL_ROOT_PASSWORD=123456 -d mysql:latest
```
